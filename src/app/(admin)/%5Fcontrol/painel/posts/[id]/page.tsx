...
export default async function EditPostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="mx-auto max-w-5xl px-6 py-10"><Link href="/_control/painel/posts" className="text-sm text-muted-foreground">← Voltar aos posts</Link><h1 className="mt-5 text-3xl font-semibold">Editar post</h1><p className="mt-2 text-sm text-muted-foreground">{post.status === 2 ? "Publicado" : post.status === 3 ? "Arquivado" : "Rascunho"}</p><PostForm post={post} categories={categories} tags={tags} action={action} /></main>
}
