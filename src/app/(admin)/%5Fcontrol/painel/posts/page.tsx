...
function PostThumbnail({ url }: { url: string | null }) {
  if (!url) {
    return <div className="flex h-14 w-20 shrink-0 items-center justify-center rounded-md border border-dashed border-white/10 text-[10px] text-muted-foreground">Sem capa</div>
  }
  return <div className="h-14 w-20 shrink-0 overflow-hidden rounded-md border border-white/10 bg-black/20">
    {/* eslint-disable-next-line @next/next/no-img-element -- URL dinâmica administrada pelo backend */}
...
export default async function AdminPostsPage({ searchParams }: Props) {
  ...
  const filtered = posts.filter((post) =>
    (!search || `${post.title} ${post.slug}`.toLocaleLowerCase("pt-BR").includes(search)) &&
  ...
  return <main className="mx-auto max-w-6xl px-6 py-10">
    ...
    <form className="mt-8 grid gap-3 sm:grid-cols-[1fr_12rem_auto]">
      <input name="busca" defaultValue={query.busca} placeholder="Buscar por título ou slug" className="rounded-md border border-white/10 bg-black/20 px-3 py-2 text-sm" />
      <select name="status" defaultValue={query.status ?? ""} className="rounded-md border border-white/10 bg-[#11100f] px-3 py-2 text-sm"><option value="">Todos os status</option><option value="1">Rascunhos</option><option value="2">Publicados</option><option value="3">Arquivados</option></select>
      <button className="rounded-md border border-white/10 px-4 py-2 text-sm">Filtrar</button>
    </form>
    {query.erro && <p role="alert" className="mt-4 text-sm text-red-300">{query.erro}</p>}
    <div className="surface mt-5 overflow-hidden rounded-lg border">
      {filtered.length ? <ul className="divide-y divide-white/10">{filtered.map((post) => <li key={post.id} className="grid gap-4 px-5 py-4 md:grid-cols-[1fr_auto] md:items-center">
        <div className="flex min-w-0 items-start gap-4">
          <PostThumbnail url={post.coverImageUrl} />
          <div className="min-w-0">
            {post.status === ADMIN_POST_STATUS.archived ? <span className="font-medium">{post.title}</span> : <Link href={`/_control/painel/posts/${post.id}`} className="font-medium hover:text-[#f0dfbd]">{post.title}</Link>}
            {post.excerpt && <p className="mt-1 line-clamp-2 text-sm text-muted-foreground">{post.excerpt}</p>}
            <p className="mt-1 text-xs text-muted-foreground">/{post.slug} · {labels[post.status]}</p>
          </div>
        </div>
      ...
      </li>)}</ul> : <p className="p-8 text-sm text-muted-foreground">Nenhum post encontrado.</p>}
    </div>
  </main>
}
